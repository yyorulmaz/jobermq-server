...
namespace JoberMQ.Implementation.Distributor.Default
{
    internal class DefaultMessageDistributorSearchQueueTag : MessageDistributorBase
    {
        ...
        public override async Task<ResponseModel> Distributoring(MessageDbo message)
        {
            ...
            {
                foreach (string tag in que.Tags)
                {
                    if (Regex.IsMatch(tag, message.Message.Routing.RoutingKey))
                    {
