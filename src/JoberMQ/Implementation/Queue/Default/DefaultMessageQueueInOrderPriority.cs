    ...
    internal class DefaultMessageQueueInOrderPriority : MessageQueueBase
    {
        ...
        protected override void Qperation()
        {
            foreach (var message in messageChilds.ChildData.OrderByDescending(x => x.Value.Message.PriorityType))
            {
                ...
                if (client != null)
                {
                    JoberHost.JoberMQ.JoberHubContext.Clients.Client(client.ConnectionId).SendCoreAsync("ReceiveMessage", new[] { message.Value }).ConfigureAwait(false);
