...
namespace JoberMQ.Server.Implementation.Queue
{
    ...
    {
        IQueueChildDataBaseFIFO Data;

        public DfQueueFIFO(BrokerConfigModel brokerConfig, string distributorName, string queueName, MatchTypeEnum matchType, SendTypeEnum sendType, IClientGroup clientGroup, IQueueDataBase queueDataBase) : base(brokerConfig, distributorName, queueName, matchType, sendType, clientGroup, queueDataBase)
        {
            Data = QueueChildDataBaseFactory.CreateQueueChildDataBaseFIFO(brokerConfig.QueueChildFIFOFactory, queueDataBase);
