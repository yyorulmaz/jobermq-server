...
{
    internal class DfMessageQueueLIFO<THub> : MessageQueueBase where THub : Hub
    {
        IChildMemLIFORepository<Guid,MessageDbo> ChildData;
        IHubContext<THub> context;
        public DfMessageQueueLIFO(IConfigurationQueue configurationQueue, string queueKey, MatchTypeEnum matchType, SendTypeEnum sendType, PermissionTypeEnum permissionType, bool isDurable, IClientGroup clientGroup, IMemRepository<Guid, MessageDbo> queueDataBase, IMessageDbOpr messageDbOpr, IHubContext<THub> context) : base(configurationQueue, queueKey, matchType, sendType, permissionType, isDurable, clientGroup, queueDataBase, messageDbOpr)
        {
            ChildData = QueueChildDataBaseFactory.CreateQueueChildDataBaseLIFO(configurationQueue.QueueChildLIFOFactory, queueDataBase);
